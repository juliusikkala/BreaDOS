...
void init_text_devices()
{
    ...
    {
        text_devices_count = 1;
    }
}

void deinit_text_devices()
{
    for(size_t i = 0; i < text_devices_count; ++i)
    {
        text_devices[i].driver->deinit(text_devices + i);
    }
    text_devices_count = 0;
}

size_t get_text_device_count()
{
    return text_devices_count;
}
...
size_t get_text_devices(
    ...
    if(count > text_devices_count) count = text_devices_count;
